# OpenCode 监控技术方案 - 补充调研报告

> 调研日期：2026-04-13（第二轮深度调研）
> 目标：基于 GitHub 开源项目深入调研，补充可真正实现的 OpenCode 监控方案
...
### 方案 A：cctop 的 Plugin Hook + 文件监听（⭐ 最推荐参考）
...
**1. OpenCode 插件（cctop.js）**

```javascript
// 位置：~/.config/opencode/plugins/cctop.js
export const name = "cctop"
...
**3. 文件写入格式（~/.cctop/sessions/{sessionID}.json）**
...
### 方案 B：opencode-monitor 的 Plugin + WebSocket（⭐ 实时性最强）
...
### 方案 C：OpenCode REST API + SSE（官方支持）
...
| 模块 | 原评估 | 更新后评估 |
|------|--------|-----------|
| OpenCode 监控 | 0% 可复用 | **70% 可复用**（参考 cctop + opencode-monitor） |
| 总体可复用度 | 约 75% | **约 78%** |
